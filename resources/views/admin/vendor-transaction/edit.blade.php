...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Vendor Transaction /</span> Edit</h4>
                            ...
                            <select class="form-control" required name="vendorId">
                                ...
                                @foreach ($users as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('vendorId', $transaction->vendorId) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control" required name="transactionType">
                                ...
                                @foreach ($type as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('transactionType', $transaction->transactionType) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="transactionDate" class="form-control"
                                value="{{ old('transactionDate', $transaction->transactionDate) }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" required name="status">
                                ...
                                @foreach (transactionStatus() as $item)
                                    <option value="{{ $item }}"
                                        {{ old('status', $transaction->status) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
